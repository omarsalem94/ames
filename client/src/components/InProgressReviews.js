...
const InProgressReviews = () => {
  ...
  useEffect(() => {
    const fetchModules = async () => {
      try {
        ...
        setModules(
          response.data.filter((module) => module.status === "In Progress")
        );
      } catch (error) {
    ...
    const fetchPrograms = async () => {
      try {
        ...
        setPrograms(
          response.data.filter((program) => program.status === "In Progress")
        );
      } catch (error) {
  ...
  const handleViewModule = (id) => {
    navigate(`/module-leader/review/module/${id}`);
  };

  const handleViewProgram = (id) => {
    navigate(`/module-leader/review/program/${id}`);
  };
  ...
  return (
    <div>
      ...
      <div className="container mx-auto p-6">
        ...
        <div>
          <h2 className="text-3xl font-semibold mb-6 text-gray-800">Modules</h2>
          {modules.length === 0 ? (
            <p className="text-gray-500">No in-progress modules found.</p>
          ) : (
                ...
                <tbody>
                  {modules.map((module) => (
                    <tr key={module._id} className="hover:bg-gray-100">
                      ...
                      <td className="py-3 px-6 border-b">{module.fullName}</td>
                      <td className="py-3 px-6 border-b">
                        {module.facultyCode}
                      </td>
                      <td className="py-3 px-6 border-b">{module.status}</td>
        ...
        <div className="mt-10">
          ...
          {programs.length === 0 ? (
            <p className="text-gray-500">No in-progress programs found.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full bg-white shadow-md rounded-lg">
                <thead>
                  <tr className="bg-blue-600 text-white">
                    <th className="py-3 px-6 border-b text-left">Route Code</th>
                    <th className="py-3 px-6 border-b text-left">
                      Program Full Name
                    </th>
                    <th className="py-3 px-6 border-b text-left">School</th>
                ...
                <tbody>
                  {programs.map((program) => (
                    <tr key={program._id} className="hover:bg-gray-100">
                      ...
                      <td className="py-3 px-6 border-b">{program.fullName}</td>
                      <td className="py-3 px-6 border-b">
                        {program.facultyCode}
                      </td>
                      <td className="py-3 px-6 border-b">{program.status}</td>
                      <td className="py-3 px-6 border-b flex space-x-2">
                        <button
